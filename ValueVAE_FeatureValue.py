...
class EncoderNet_2(nn.Module):
    def __init__(self,N,middleDim):
        super(EncoderNet_2,self).__init__()
        self.N = N
...
class ClfModel_ColorShape(nn.Module):
    def __init__(self,inputDim,numShapeClasses=4,numColorClasses=4,drp_out_param=0.7):
      super(ClfModel_ColorShape,self).__init__()
      ...
      #     nn.Linear(30,20)
      # )
      self.lin_color = nn.Linear(20,numColorClasses)
      self.lin_shape = nn.Linear(20,numShapeClasses)
      self.act  = nn.Softmax(dim=1)
      self.clf_loss =nn.CrossEntropyLoss()
    def produce_latent(self,x):
      return self.lin_stack(x)
    def forward(self,x,val_c,val_s):
      out = self.lin_stack(x)
      out2_c  = self.lin_color(out)
      out3_c  = self.act(out2_c)
      loss_color = self.clf_loss(out3_c,val_c)
      out2_s  = self.lin_shape(out)
      out3_s  = self.act(out2_s)
      loss_shape = self.clf_loss(out3_s,val_s)
...
class BlockSwapVAE_N_CS(nn.Module):
    def __init__(self,encoder,decoder,N,beta,alpha,device,embed_file,embed_flag = True,inv_cov = None,numClasses=None,numUnits=78,FreezeFlag = True):#,numCountVals
        super(BlockSwapVAE_N_CS,self).__init__()
        self.device = device
        ...
        self.beta = beta
        self.alpha = alpha
        self.embed = ClfModel_ColorShape(numUnits)#Model2(latentDim=20,inputDim=20,inv_cov = self.inv_cov) #20 27
    ...
    def randTrfm(self,x):
      ...
      new_x[torch.FloatTensor(x.shape).uniform_() > 0.5] = 0 #0.6 0.3
