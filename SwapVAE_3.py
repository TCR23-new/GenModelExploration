...
class EncoderNet(nn.Module):
    def __init__(self,N,middleDim):#,numCountVals
        ...
        self.lin1 = nn.Linear(self.N,self.middleDim) # nn.Identity()
...
class DecoderNet(nn.Module):
    def __init__(self,N,middleDim,valCats = None):#,numCountVals
        super(DecoderNet,self).__init__()
        self.N = N
        # self.numCountVals = numCountVals
        self.middleDim = middleDim
        if valCats != None:
          self.lin1 = nn.Linear(33+valCats,self.middleDim)
        else:
        ...
        self.act1 = nn.LeakyReLU(1.0) #nn.ReLU()
    ...
    def forward(self,x):
        ...
        out23 = self.act1(self.nrm4(self.lin23(out22)))
        out24 = self.act1(self.nrm5(self.lin24(out23)))
        out3 = self.act1(self.lin3(out24))
        return out3
...
class BlockSwapVAE_N(nn.Module):
    ...
    def randTrfm(self,x):
      new_x = torch.clone(x)
      new_x[torch.FloatTensor(x.shape).uniform_() > 0.6] = 0 #0.6 0.7
      noise = torch.normal(0,1,size = new_x.shape)
      return new_x # + 0.5*noise.to(torch.device("cuda"))
